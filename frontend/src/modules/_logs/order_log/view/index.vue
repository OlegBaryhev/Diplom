<template>
  <div class="log-page min-h-screen w-full">
    ...
    <SidePanel
      ...
      @delete="itemToDelete = $event; modalStore.open('removeOrderLog')"
    />
...
import { getOrderLogsRequest, deleteOrderLogRequest } from '../api';
...
const fetchLogs = debounce(async () => {
  ...
  try {
    const result = await getOrderLogsRequest({
...
const deleteItem = async () => {
  try {
    await deleteOrderLogRequest(itemToDelete.value?.id);
