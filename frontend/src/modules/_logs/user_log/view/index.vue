  ...
  <div class="log-page min-h-screen w-full">
    <div class="log-page__content w-full h-full">
      <VFixedHeaderNTable
        ...
        title="Логи таблицы Пользователи"
        show-total
        :total="logs.length"
        :items="logs"
        :loading="loading"
    ...
    <SidePanel
      ...
      @delete="itemToDelete = $event; modalStore.open('removeUserLog')"
    />
...
import { getUserLogsRequest, deleteUserLogRequest } from '../api';
...
const fetchLogs = debounce(async () => {
  ...
  try {
    const result = await getUserLogsRequest({
...
const deleteItem = async () => {
  try {
    await deleteUserLogRequest(itemToDelete.value?.id);
