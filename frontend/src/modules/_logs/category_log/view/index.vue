<template>
  <div class="log-page min-h-screen w-full">
    ...
    <SidePanel
      ...
      @delete="itemToDelete = $event; modalStore.open('removeCategoryLog')"
    />

    <VConfirmationModal
      id="removeCategoryLog"
...
import { getCategoryLogsRequest, deleteCategoryLogRequest } from '../api';
...
const fetchLogs = debounce(async () => {
  ...
  try {
    const result = await getCategoryLogsRequest({
...
const deleteItem = async () => {
  try {
    await deleteCategoryLogRequest(itemToDelete.value?.id);
