<template>
  <div class="log-page min-h-screen w-full">
    ...
    <SidePanel
      ...
      @delete="itemToDelete = $event; modalStore.open('removeProductLog')"
    />
...
import { getProductLogsRequest, deleteProductLogRequest } from '../api';
...
const fetchLogs = debounce(async () => {
  ...
  try {
    const result = await getProductLogsRequest({
...
const deleteItem = async () => {
  try {
    await deleteProductLogRequest(itemToDelete.value?.id);
